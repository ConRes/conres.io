...
const CLASSES_BASE = 'classes/baseline'; // Change to 'classes/new' to test new implementation in isolation
...
async function assignProfileToDocument(pdfDocument, profileBytes, options) {
    const { PDFName, PDFArray, PDFDict, PDFRef, PDFRawStream } = await import('../packages/pdf-lib/pdf-lib.esm.js');
...
async function main() {
    ...
    if (options.useLegacy) {
        ...
        await import(join(__dirname, 'internal', 'legacy', 'convert-pdf-color.js'));
        return;
    }
    ...
    if (options.useWorkers) {
        ...
        // Match WorkerPool.getDefaultWorkerCount(): Math.max(1, Math.floor(cpus / 2))
        const actualWorkerCount = Math.max(1, Math.floor(os.cpus().length / 2));
    ...
    // Load dependencies
    const { PDFDocument } = await import('../packages/pdf-lib/pdf-lib.esm.js');
    const { PDFDocumentColorConverter } = await import(`../${CLASSES_BASE}/pdf-document-color-converter.js`);
    const { DiagnosticsCollector } = await import(`../classes/diagnostics/diagnostics-collector.js`);
    const { MainDiagnosticsCollector } = await import(`../classes/diagnostics/main-diagnostics-collector.js`);
    ...
    const converter = new PDFDocumentColorConverter({
        ...
        verbose: options.verbose,
        diagnostics,
        colorEnginePath: colorEnginePath?.replace?.(/^\.\.\//, '../../'),
        engineVersion,
        // Output format overrides
