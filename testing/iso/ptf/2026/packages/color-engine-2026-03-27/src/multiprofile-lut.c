...
cmsInt32Number CompositeLUTSamplerFloat(
    ...
    if (cargo->inputColorSpace == cmsSigCmykData) {
        ...
        // Lab V4 encoding (matches cmsLabEncoded2FloatV4 in cmspcs.c):
        //   L = wLab[0] / 652.8    (0→0.0, 65280→100.0)
        //   a = wLab[1] / 256 - 128 (0→-128.0, 32768→0.0, 65280→127.0)
        //   b = wLab[2] / 256 - 128
        //
        // This ensures grid point 8 of 17 (In[1]=32768) maps to a*=0.0 exactly,
        // so the K-Only sampler correctly detects neutral Lab and produces CMY=0.
        //
        // NOTE: LittleCMS's CLUT evaluation normalizes Lab using (a+128)/255,
        // which maps a*=0 to 0.50196 instead of grid point 0.5. This causes ~3%
        // interpolation leakage from neighboring non-neutral grid points during
        // evaluation. The post-sampling achromatic coercion below compensates.
        inputFloat[0] = (cmsFloat32Number)In[0] / 652.8f;
        inputFloat[1] = ((cmsFloat32Number)In[1] / 256.0f) - 128.0f;
        inputFloat[2] = ((cmsFloat32Number)In[2] / 256.0f) - 128.0f;
    } else {
    ...
    // but Lab uses L=0-100, a/b=-128 to +127
    {
        ...
        if (cargo->outputColorSpace == cmsSigLabData) {
            // Lab V4 encoding (matches cmsFloat2LabEncodedV4 in cmspcs.c):
            //   wLab[0] = L * 652.8   (0.0→0, 100.0→65280)
            //   wLab[1] = (a + 128) * 256 (0.0→32768)
            //   wLab[2] = (b + 128) * 256
            ...
            Out[0] = _cmsQuickSaturateWord(L * 652.8f);
            Out[1] = _cmsQuickSaturateWord((a + 128.0f) * 256.0f);
            Out[2] = _cmsQuickSaturateWord((b + 128.0f) * 256.0f);

            // Achromatic coercion: pure black/white must have neutral a/b
            if (Out[0] == 0 || Out[0] == 65280) {
                Out[1] = 32768;  // a* = 0 in Lab V4 encoding
                Out[2] = 32768;  // b* = 0 in Lab V4 encoding
            }
...
static cmsHTRANSFORM CreateKOnlyGCRMultiprofileTransform(
    ...
    if (nProfiles == 2) {
        ...
        //
        // PIPELINE CONCATENATION approach: instead of sampling Lab→sRGB→CMYK into
        // a single CLUT (which introduces Lab encoding mismatch artifacts), we
        // concatenate two pipelines:
        //   1. Lab → sRGB (analytical, from _cmsDefaultICCintents — preserves neutrals exactly)
        //   2. sRGB → CMYK K-Only (CLUT from BlackPreservingKOnlyGCRIntents —
        //      tetrahedral interpolation preserves R=G=B neutrals)
        //
        // The combined pipeline evaluates Lab→sRGB analytically then sRGB→CMYK via
        // CLUT. Neutral Lab (a*=0, b*=0) → exact R=G=B → exact K-only CMYK.
        ...
        // 1. Get Lab → sRGB pipeline (analytical: matrix + curves, no CLUT)
        cmsUInt32Number frontIntents[2] = { INTENT_RELATIVE_COLORIMETRIC, INTENT_RELATIVE_COLORIMETRIC };
        cmsBool frontBPC[2] = { TRUE, TRUE };
        cmsFloat64Number frontAdapt[2] = { 1.0, 1.0 };
        cmsHPROFILE frontProfiles[2] = { hProfiles[0], hSRGBProfile };

        cmsPipeline* frontPipeline = _cmsDefaultICCintents(
            ContextID, 2, frontIntents, frontProfiles,
            frontBPC, frontAdapt, intermediateFlags
        );
        ...
        // 2. Get sRGB → CMYK K-Only pipeline (CLUT populated by KOnlyGCRSampler3D)
        cmsHPROFILE kOnlyProfiles[2] = { hSRGBProfile, hProfiles[1] };
        cmsUInt32Number kOnlyIntents[2] = { INTENT_PRESERVE_K_ONLY_RELATIVE_COLORIMETRIC_GCR, INTENT_PRESERVE_K_ONLY_RELATIVE_COLORIMETRIC_GCR };
        cmsBool kOnlyBPC[2] = { TRUE, TRUE };
        cmsFloat64Number kOnlyAdapt[2] = { 1.0, 1.0 };

        kOnlyPipeline = BlackPreservingKOnlyGCRIntents(
            ContextID, 2, kOnlyIntents, kOnlyProfiles,
            kOnlyBPC, kOnlyAdapt, dwFlags
        );

        if (kOnlyPipeline == NULL) {
            cmsPipelineFree(frontPipeline);
            cmsCloseProfile(hSRGBProfile);
            return NULL;
        }

        // 3. Concatenate: Lab→sRGB stages + sRGB→CMYK CLUT stage
        combinedPipeline = cmsPipelineDup(frontPipeline);
        cmsPipelineFree(frontPipeline);
        ...
        if (!cmsPipelineCat(combinedPipeline, kOnlyPipeline)) {
        ...
        // Create transform from concatenated pipeline.
        // IMPORTANT: Pass NOOPTIMIZE to prevent _cmsOptimizePipeline from
        // merging the analytical Lab→sRGB stages into a single CLUT, which
        // would reintroduce the Lab encoding mismatch artifacts.
        cmsUInt32Number inputFormatCopy = InputFormat;
        cmsUInt32Number outputFormatCopy = OutputFormat;
        cmsUInt32Number dwFlagsCopy = dwFlags | cmsFLAGS_NOOPTIMIZE;
    ...
    // For 3+ profile chains — PIPELINE CONCATENATION approach
    //
    // Same strategy as 2-profile: concatenate analytical front-stage pipelines
    // with the K-Only CLUT pipeline, avoiding combined CLUT sampling that
    // introduces Lab encoding mismatch artifacts.
    //
    // Build: profiles[0→1] ++ profiles[1→2] ++ ... ++ [optional →sRGB] ++ K-Only CLUT
    ...
    // 1. Build front-stage pipeline by concatenating 2-profile segments
    combinedPipeline = NULL;
    {
        cmsUInt32Number frontIntents[2] = { INTENT_RELATIVE_COLORIMETRIC, INTENT_RELATIVE_COLORIMETRIC };
        cmsBool frontBPC[2] = { TRUE, TRUE };
        cmsFloat64Number frontAdapt[2] = { 1.0, 1.0 };

        // Chain profiles [0→1], [1→2], ..., [n-3→n-2]
        for (cmsUInt32Number i = 0; i < nProfiles - 2; i++) {
            cmsHPROFILE segProfiles[2] = { hProfiles[i], hProfiles[i + 1] };

            cmsPipeline* segment = _cmsDefaultICCintents(
                ContextID, 2, frontIntents, segProfiles,
                frontBPC, frontAdapt, intermediateFlags
            );

            if (segment == NULL) {
                if (combinedPipeline) cmsPipelineFree(combinedPipeline);
                if (hSRGBProfile) cmsCloseProfile(hSRGBProfile);
                return NULL;
            }

            if (combinedPipeline == NULL) {
                combinedPipeline = segment;
            } else {
                if (!cmsPipelineCat(combinedPipeline, segment)) {
                    cmsPipelineFree(combinedPipeline);
                    cmsPipelineFree(segment);
                    if (hSRGBProfile) cmsCloseProfile(hSRGBProfile);
                    return NULL;
                }
                cmsPipelineFree(segment);
        ...
        // If sRGB intermediate needed, add [n-2→sRGB] segment
        if (insertSRGB) {
            cmsHPROFILE segProfiles[2] = { hProfiles[nProfiles - 2], hSRGBProfile };
    ...
    // 2. Build K-Only sRGB → CMYK pipeline
    {
        cmsHPROFILE kOnlyInputProfile = insertSRGB ? hSRGBProfile : hProfiles[nProfiles - 2];
        cmsHPROFILE kOnlyProfiles[2] = { kOnlyInputProfile, hProfiles[nProfiles - 1] };
        cmsUInt32Number kOnlyIntents[2] = { INTENT_PRESERVE_K_ONLY_RELATIVE_COLORIMETRIC_GCR, INTENT_PRESERVE_K_ONLY_RELATIVE_COLORIMETRIC_GCR };
        cmsBool kOnlyBPC[2] = { TRUE, TRUE };
        cmsFloat64Number kOnlyAdapt[2] = { 1.0, 1.0 };
        ...
        // 3. Concatenate front-stage + K-Only
        if (!cmsPipelineCat(combinedPipeline, kOnlyPipeline)) {
            cmsPipelineFree(combinedPipeline);
            cmsPipelineFree(kOnlyPipeline);
            if (hSRGBProfile) cmsCloseProfile(hSRGBProfile);
    ...
    // 4. Create transform from concatenated pipeline with NOOPTIMIZE
    //    to prevent merging analytical stages into a single CLUT
    {
        cmsUInt32Number inputFormatCopy = InputFormat;
        cmsUInt32Number outputFormatCopy = OutputFormat;
        cmsUInt32Number dwFlagsCopy = dwFlags | cmsFLAGS_NOOPTIMIZE;
