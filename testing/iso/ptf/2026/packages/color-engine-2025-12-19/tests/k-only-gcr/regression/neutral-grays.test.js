...
describe('Regression: Neutral Grays → K-Only', () => {
   ...
   */
  async function testNeutralGray(profilePath, rgb) {
    // Load profile
    ...
    const C = Math.round(output[0] / 255 * 1000) / 10;  // 1 decimal place
    const M = Math.round(output[1] / 255 * 1000) / 10;
    const Y = Math.round(output[2] / 255 * 1000) / 10;
    const K = Math.round(output[3] / 255 * 1000) / 10;

    return { C, M, Y, K };
  }

  describe('CoatedFOGRA39 (Representative Profile)', () => {
    const profilePath = join(__dirname, '../../fixtures/profiles/cmyk/CoatedFOGRA39.icc');

    for (const gray of NEUTRAL_GRAYS) {
      it(`should convert ${gray.name} to K-only`, async () => {
        const result = await testNeutralGray(profilePath, gray.rgb);
  ...
  describe('UncoatedFOGRA29 (Was Problematic)', () => {
    const profilePath = join(__dirname, '../../fixtures/profiles/cmyk/UncoatedFOGRA29.icc');
  ...
  describe('Multiple Profiles - Neutral Gray Guarantee', () => {
    ...
    for (const profileName of profiles) {
      describe(profileName, () => {
        const profilePath = join(__dirname, '../../fixtures/profiles/cmyk', profileName);
