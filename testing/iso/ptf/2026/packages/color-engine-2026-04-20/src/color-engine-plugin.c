...
static void ColorEngineDoTransform(
    ...
    if (g_inColorEngineTransform) {
        ...
        /*
         * Lab 16-bit float promotion: force float pipeline evaluation for Lab
         * 16-bit input (TYPE_Lab_16, TYPE_Lab_16_SE, TYPE_LabV2_16, etc.).
         *
         * LittleCMS's 16-bit pipeline evaluation path introduces chromaticity
         * errors for neutral Lab values when evaluating analytical stages
         * (matrices + curves). The float path preserves neutrals exactly.
         *
         * Strategy: use the 16-bit input formatter to decode raw bytes (handles
         * endianness/SE), then convert from Lab16 wire format to pipeline float
         * wire format, and evaluate via cmsPipelineEvalFloat.
         *
         * Lab V4 (PT_Lab):   pipeline_float = lab16_wire / 65280.0
         * Lab V2 (PT_LabV2): pipeline_float = lab16_wire / 65535.0
         */
        cmsUInt32Number inputColorSpace = T_COLORSPACE(inputFormat);
        /* Promote Lab16 → float when the K-Only concatenated pipeline signals it
         * via cmsFLAGS_LAB16_FLOAT_PROMOTION. Standard Lab transforms use the
         * normal 16-bit path for backward compatibility. */
        cmsBool isLab16 = isLabColorSpace(inputColorSpace) && !T_FLOAT(inputFormat)
            && (CMMcargo->dwOriginalFlags & cmsFLAGS_LAB16_FLOAT_PROMOTION);

        if (T_FLOAT(inputFormat) || isLab16) {
            /* Float path (also used for Lab 16-bit with float promotion) */
            ...
            if (isLab16) {
                /* Lab 16-bit: read with 16-bit formatter, promote to float,
                 * evaluate pipeline in float, convert back to 16-bit for output */
                cmsFormatter16 fromInput16, toOutput16;
                _cmsGetTransformFormatters16(CMMcargo, &fromInput16, &toOutput16);

                cmsUInt16Number wIn[cmsMAXCHANNELS], wOut[cmsMAXCHANNELS];
                memset(wIn, 0, sizeof(wIn));
                memset(wOut, 0, sizeof(wOut));

                /* V4 (PT_Lab): 65280.0, V2 (PT_LabV2): 65535.0 */
                cmsFloat32Number labScale = (inputColorSpace == PT_LabV2) ? 65535.0f : 65280.0f;

                cmsUInt32Number outputChannels = T_CHANNELS(CMMcargo->OutputFormat);
                ...
                for (cmsUInt32Number line = 0; line < LineCount; line++) {
                    ...
                    for (cmsUInt32Number pixel = 0; pixel < PixelsPerLine; pixel++) {
                        lineAccum = fromInput16(CMMcargo, wIn, lineAccum, Stride->BytesPerPlaneIn);
                        /* Lab16 wire → pipeline float */
                        fIn[0] = (cmsFloat32Number)wIn[0] / labScale;
                        fIn[1] = (cmsFloat32Number)wIn[1] / labScale;
                        fIn[2] = (cmsFloat32Number)wIn[2] / labScale;
                        cmsPipelineEvalFloat(fIn, fOut, CMMcargo->Lut);
                        /* Pipeline float [0,1] → 16-bit wire [0,65535] */
                        for (cmsUInt32Number ch = 0; ch < outputChannels; ch++) {
                            cmsFloat32Number v = fOut[ch];
                            if (v < 0.0f) v = 0.0f;
                            if (v > 1.0f) v = 1.0f;
                            wOut[ch] = (cmsUInt16Number)(v * 65535.0f + 0.5f);
                        }
                        lineOutput = toOutput16(CMMcargo, wOut, lineOutput, Stride->BytesPerPlaneOut);
                    }
    ...
    if (data != NULL && !data->pipelineOptimized) {
        ...
        // Skip optimization for Lab16 float promotion transforms — the concatenated
        // pipeline (analytical Lab→sRGB + K-Only CLUT) must not be merged
        // into a single CLUT. The float promotion in the re-entry path
        // evaluates the concatenated stages correctly.
        if (flags & cmsFLAGS_LAB16_FLOAT_PROMOTION) {
