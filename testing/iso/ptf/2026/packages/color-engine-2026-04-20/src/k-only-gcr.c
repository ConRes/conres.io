...
/*
 * Engine parameters (module-level).
 *
 * Mirrors `packages/color-engine/src/index.js` — the JS-side setters
 * validate and push here. One WASM module instance per `createEngine()`
 * call, so this state is isolated per engine.
 *
 * Defaults reproduce `color-engine-2026-03-27` byte-for-byte.
 */
/*
 * Engine-parameters API surface.
 *
 * The parameter bag is intentionally empty. The `set` / `reset` entry
 * points exist so new parameters can be added without changing the
...
static const cmsUInt32Number dwFlagsApplyBPC = 
    0
    ...
    cmsCIELab* outKOnlyBlackpointLab)
{
    ...
    hLab2CMYK = cmsCreateTransformTHR(
        ...
        cmsFLAGS_BLACKPOINTCOMPENSATION | dwFlagsApplyBPC
    );
    ...
    // // 3b. Optional: default K-only black-point scale approximation.
    // //
    // // Set by JS via `engine.setParameters({ defaultKOnlyBlackpointScaleApproximation: true })`.
    // // When enabled, probe the profile's K response at K = 99 and K = 99.5
    // // through the same standard transform (Relative Colorimetric + BPC +
    // // NOOPTIMIZE), linearly extrapolate to estimate the L* that K = 100
    // // "ought" to have been (`L_100_approx = 2·L_995 − L_99`), and replace
    // // `yKOnly` with `Y(Lab(L_100_approx, 0, 0))`. Only yKOnly changes;
    // // yBlack and the rest of the formula stay as in 2026-03-27.
    // if (g_defaultKOnlyBlackpointScaleApproximation) {
    //     /// DO NOT REMOVE STILL PROTOTYPING ///

    //     cmsFloat64Number cmyk985[4] = { 0.0, 0.0, 0.0, 98.5 };
    //     cmsFloat64Number cmyk99[4]  = { 0.0, 0.0, 0.0, 99.0 };
    //     cmsFloat64Number cmyk995[4] = { 0.0, 0.0, 0.0, 99.5 };
    //     cmsFloat64Number cmyk100[4] = { 0.0, 0.0, 0.0, 100.0 };
    //     cmsCIELab lab985, lab99, lab995, lab100;
    //     cmsDoTransform(hCMYK2Lab, cmyk985, &lab985, 1);
    //     cmsDoTransform(hCMYK2Lab, cmyk99,  &lab99,  1);
    //     cmsDoTransform(hCMYK2Lab, cmyk995, &lab995, 1);
    //     cmsDoTransform(hCMYK2Lab, cmyk100, &lab100, 1);

    //     cmsFloat64Number L_100_approx = 2 * lab995.L - lab99.L;
        
    //     // cmsFloat64Number L_100_approx = lab99.L; // - (lab985.L - lab995.L);

    //     ///

    //     // cmsFloat64Number cmykLimit[4] = { 0, 0, 0, 100 };
    //     // cmsCIELab labTarget, labReference;

    //     // cmsDoTransform(hCMYK2Lab, cmykLimit, &labTarget, 1);
    //     // cmsDoTransform(hCMYK2Lab, cmykLimit, &labReference, 1);

    //     // // while (labTarget.L <= estimatedKOnlyBlackpointLab.L + 3) {
    //     // while (labTarget.L <= labReference.L + 10) {
    //     //     cmykLimit[3] -= 0.5; // Move from 100 to 99.5
    //     //     cmsDoTransform(hCMYK2Lab, cmykLimit, &labTarget, 1);
    //     // }

        
    //     // cmsFloat64Number L_100_approx = labTarget.L > estimatedKOnlyBlackpointLab.L
    //     //     ? labTarget.L
    //     //     : estimatedKOnlyBlackpointLab.L;

    //     ///
    //     // cmsFloat64Number L_100_approx = estimatedKOnlyBlackpointLab.L + (labTarget.L - labReference.L);
    //     ///

    //     // cmsFloat64Number L_100_approx = estimatedKOnlyBlackpointLab.L - 10;
    //     // cmsFloat64Number L_100_approx = 20;

    //     if (L_100_approx < 0.0) L_100_approx = 0.0;
    //     if (L_100_approx > 100.0) L_100_approx = 100.0;

    //     cmsCIELab approxLab = { L_100_approx, 0.0, 0.0 };
    //     cmsCIEXYZ approxXYZ;
    //     cmsLab2XYZ(NULL, &approxXYZ, &approxLab);
    //     kOnlyBlackXYZ.Y = approxXYZ.Y;

    //     #if K_ONLY_PARITY_DEBUGGING
    //     fprintf(stderr, "  [R2 approximation] L_99=%.6f, L_995=%.6f, L_100_approx=%.6f, Y_approx=%.6f\n",
    //             lab99.L, lab995.L, L_100_approx, approxXYZ.Y);
    //     #endif
    // }
    ...
    //
    // Degeneracy correction (Step 1 of the 2026-04-15 detour):
    //
    // The internal `hCMYK2Lab` transform used above is created with
    // Relative Colorimetric + `cmsFLAGS_BLACKPOINTCOMPENSATION`, which
    // collapses both the rich-black point and the K-only dark point
    // toward `Lab(0, 0, 0)` for profiles whose rich-black and K=100
    // land at nearly the same perceptual black. In that case
    // `yBlack ≈ yKOnly ≈ 0` and `(1 - yKOnly) / (1 - yBlack) → 1.0`,
    // which makes `ApplyKOnlyBlackpointCompensation` an identity
    // transform and the sampler's iteration target becomes `L* = 0`
    // — unreachable, so the iteration converges at `K_float < K_MAX`
    // and the corner LUT entry encodes to `uint8 = 254` instead of
    // `255`. Pattern C in the 2026-04-15 diagnostic.
    //
    // Fix: when the formula collapses to `≈ 1.0` *and* the BPC-flattened
    // points are both essentially zero, substitute the profile's RAW
    // `Y` at `CMYK(0, 0, 0, 100)` (measured without BPC) into the
    // `1 - yKOnly` numerator. That yields a scale < 1 whose custom BPC
    // maps `Lab(0, 0, 0)` onto the profile's physical K-only ceiling,
    // giving the iteration a reachable target.
    {
        cmsFloat64Number yBlack = blackXYZ.Y;
        cmsFloat64Number yKOnly = kOnlyBlackXYZ.Y;

        // Avoid division by zero
        if (fabs(1.0 - yBlack) < 1e-12) {
            scale = 1.0;
            // fprintf(stderr, "[C BPC Scale] Warning: Standard blackpoint Y is too close to 1.0, using scale=1.0\n");
        } else {
        ...
        // Degeneracy correction: both BPC-flattened blackpoints are
        // essentially zero *and* the formula yielded scale = 1.0.
        // Substitute the raw K-only ceiling into the numerator.
        const cmsBool bpcScaleIsDegenerate =
            (scale >= 1.0 - 1e-9) &&
            (yBlack < 1e-6) &&
            (yKOnly < 1e-6);

        if (bpcScaleIsDegenerate) {
            cmsHTRANSFORM hRawCMYK2Lab = cmsCreateTransformTHR(
                ContextID,
                hOutputProfile, TYPE_CMYK_FLT,
                hLabProfile, TYPE_Lab_FLT,
                INTENT_RELATIVE_COLORIMETRIC,
                cmsFLAGS_NOCACHE);
            if (hRawCMYK2Lab != NULL) {
                cmsFloat32Number rawKOnlyCMYK[4] = { 0.0f, 0.0f, 0.0f, 100.0f };
                cmsFloat32Number rawKOnlyLab[3] = { 0.0f, 0.0f, 0.0f };
                cmsDoTransform(hRawCMYK2Lab, rawKOnlyCMYK, rawKOnlyLab, 1);
                cmsDeleteTransform(hRawCMYK2Lab);

                cmsCIELab rawKOnlyBlackpointLab;
                rawKOnlyBlackpointLab.L = (cmsFloat64Number)rawKOnlyLab[0];
                rawKOnlyBlackpointLab.a = 0.0;
                rawKOnlyBlackpointLab.b = 0.0;

                cmsCIEXYZ rawKOnlyBlackXYZ;
                cmsLab2XYZ(NULL, &rawKOnlyBlackXYZ, &rawKOnlyBlackpointLab);

                cmsFloat64Number yKOnlyRaw = rawKOnlyBlackXYZ.Y;
                if (yKOnlyRaw >= 0.0 && yKOnlyRaw < 1.0) {
                    scale = 1.0 - yKOnlyRaw;
                    if (scale < 0.0) scale = 0.0;
                    if (scale > 1.0) scale = 1.0;

                    // Also update the stored K-only Lab so the sampler's
                    // `scaledKOnlyBlackpointLab` reflects the physical
                    // ceiling used by the corrected scale.
                    estimatedKOnlyBlackpointLab = rawKOnlyBlackpointLab;
                }
            }
        }
    }
    ...
    cmsBool debugEnabled)
{
    ...
    // D50: X=0.96422, Y=1.0, Z=0.82521;
    flatSourceXYZ.X = sourceXYZ.X / 0.96422;
    flatSourceXYZ.Y = sourceXYZ.Y / 1.0;
    flatSourceXYZ.Z = sourceXYZ.Z / 0.82521;
    ...
    destXYZ.Y = flatDestXYZ.Y / 1;           // No scaling for Y
...
/**
 * Post-sampling K-only neutral-approach patch (2026-04-15 detour,
 * revised 2026-04-20).
 *
 * Enforces two invariants on the finished CLUT that the sampler's
 * iteration stages cannot reliably guarantee across all profiles:
 *
 *  1. The deepest corner `(0, 0, 0)` holds `K_uint16 = 65535`.
 *     Only the single canonical deepest-black input reads as
 *     fully-saturated K in 8-bit output.
 *
 *  2. The neutral approach to the corner is a *smooth* gradient — no
 *     input adjacent to the corner (e.g. sRGB ≈ 0.055) may
 *     interpolate into the 8-bit saturation band. Concretely, the
 *     neutral grid point `(1, 1, 1)` and its three 2-axis neighbours
 *     `(1, 1, 0)`, `(1, 0, 1)`, `(0, 1, 1)` (which together dominate
 *     trilinear interpolation for neutral shadow inputs inside
 *     cell `[0..1]`) are capped at `K_uint16 = 65400` when the
 *     sampler left them at or above that value. This leaves the
 *     sampled behaviour intact for profiles whose native output is
 *     already below the cap, and blocks adjacent inputs from rounding
 *     up to `uint8 = 255` on profiles whose sampler over-converged.
 *
 * Additional monotonicity pass on the diagonal `(i, i, i)` ensures
 * the neutral ramp strictly decreases with increasing `i`. CMY
 * channels on the diagonal are not modified — the K-only sampler
 * already forces them to zero on the neutral axis.
 *
 * Non-neutral grid points (chromatic corners that do not participate
 * in neutral-shadow trilinear interpolation to a material extent) are
 * left alone so chromatic behaviour is unaffected.
 *
 * Table indexing for a 3D CLUT (RGB → CMYK, `gridPoints` per axis,
 * `4` output channels) at grid point `(a, b, c)`:
 *
 *     index = (((a * gridPoints) + b) * gridPoints + c) * 4 + channel
 *
 * Access via `_cmsStageCLutData` (see `lcms2_plugin.h` line 530).
 *
 * @param clutStage  CLUT stage populated by `cmsStageSampleCLut16bit`.
 * @param gridPoints Grid points per input dimension.
 */
static void PatchKOnlyNeutralApproach(cmsStage* clutStage, cmsUInt32Number gridPoints)
{
    if (clutStage == NULL || gridPoints < 2) return;

    _cmsStageCLutData* clutData = (_cmsStageCLutData*)cmsStageData(clutStage);
    if (clutData == NULL || clutData->HasFloatValues) return;

    cmsUInt16Number* table = clutData->Tab.T;
    if (table == NULL) return;

    const cmsUInt32Number OUTPUT_CHANNELS = 4;
    const cmsUInt32Number K = 3;
    const cmsUInt16Number K_SATURATED = 65535u;

    // Helper macro for the flat table index of grid point (a, b, c).
    #define GP(a, b, c, ch) \
        ((((cmsUInt32Number)(a) * gridPoints + (cmsUInt32Number)(b)) * gridPoints \
          + (cmsUInt32Number)(c)) * OUTPUT_CHANNELS + (cmsUInt32Number)(ch))

    // LCMS uses tetrahedral interpolation for 3D CLUTs. On the r = g = b
    // diagonal, the tetrahedron path reduces to a 2-corner interpolation
    // between `K[0, 0, 0]` and `K[1, 1, 1]`:
    //
    //     K_interp(t) = (1 − t) * K[0, 0, 0] + t * K[1, 1, 1]
    //
    // where `t` is the fractional grid position.
    //
    // The r2 test chart's last neutral row (samples 139..161) places
    // the deepest two RGB inputs *very* close to the corner. The inputs
    // are float, but `engine.doTransform` takes `Uint8Array` so the
    // values truncate to integer before entering the LUT:
    //     sample 161: RGB(0.000, 0.000, 0.000)    →  uint8 (0)
    //                 — the corner itself, t = 0
    //     sample 160: RGB(1.401, 1.401, 1.401)    →  uint8 (1)
    //                 — 16-bit = 257, t ≈ 0.0627 in cell [0..1]
    //     sample 159: RGB(14.011, 14.011, 14.011) →  uint8 (14)
    //                 — 16-bit ≈ 3599, t ≈ 0.8791 in cell [0..1]
    //
    // With `K[0, 0, 0] = 65535`, the maximum `K[1, 1, 1]` that keeps
    // sample 160's interpolated `K_uint16` at or below the 8-bit-255
    // rounding threshold (`65406`) is:
    //
    //     K_cap = (65406 − 0.9373 * 65535) / 0.0627  ≈  63301
    //
    // A slightly lower cap (`63000`, ~ 96.13 %) holds `K[1, 1, 1]` far
    // enough below the boundary that minor interpolation drift does not
    // pull sample 160 back into the saturation band. The cap applies
    // only when the sampler already left `K[1, 1, 1]` above it — most
    // profiles whose iteration produced a physically-plausible dark-
    // grey `K` response may be at or below this already.
    const cmsUInt16Number K_NEUTRAL_NEIGHBOUR_CAP = 63000u;

    // 1. Force the canonical deepest corner.
    table[GP(0, 0, 0, K)] = K_SATURATED;

    // 2. Cap `K[1, 1, 1]` so the neutral-diagonal tetrahedral
    //    interpolation has the room it needs to transition smoothly
    //    from the forced corner back to the sampler's native ramp.
    //    This is the only grid point that participates in the r = g = b
    //    tetrahedron alongside the corner.
    if (table[GP(1, 1, 1, K)] > K_NEUTRAL_NEIGHBOUR_CAP) {
        table[GP(1, 1, 1, K)] = K_NEUTRAL_NEIGHBOUR_CAP;
    }

    // 3. Monotonicity walk along the neutral diagonal `(i, i, i)`.
    //    Ensure strictly decreasing `K_uint16` as `i` grows (lighter
    //    grey). This cascades any downstream adjustment required by
    //    the cap above into the next diagonal entries only.
    cmsUInt16Number previousK = table[GP(0, 0, 0, K)];
    for (cmsUInt32Number i = 1; i < gridPoints; i++) {
        const cmsUInt32Number idx = GP(i, i, i, K);
        cmsUInt16Number currentK = table[idx];
        if (currentK >= previousK) {
            currentK = (previousK == 0) ? 0u : (cmsUInt16Number)(previousK - 1u);
            table[idx] = currentK;
        }
        previousK = currentK;
    }
    ...
    CMSREGISTER void* Cargo)
{
    ...
    // const cmsFloat64Number STEP_FACTOR = 0.125;
    const int MAX_ITERATIONS = 1000;  // Updated to match JS (transform.js line 948)
    const cmsFloat64Number K_MAX = 100 - 0.125 / 8;
    const cmsFloat64Number K_MIN = 0.125 / 8;
    const cmsFloat64Number CMY_MAX = 100 - 0.125 / 8;
    const cmsFloat64Number CMY_MIN = 0.125 / 8;


    // Stage 1: Convert 16-bit sampler input directly to Lab_FLT.
    //
    // `params->input2lab` is created with a 16-bit input type matching the
    // profile's colorspace (TYPE_RGB_16, TYPE_Lab_16, ...). LittleCMS
    // handles the encoding-specific decode so Lab16 sentinel midpoints
    // (`0x8080` for a = b = 0) do not leak into chroma.
    cmsFloat32Number labFloat[3];
    cmsDoTransform(params->input2lab, In, labFloat, 1);

    // Runtime debugging: Check if this is a test color we want to debug.
    // Preserved for downstream diagnostics — decode once here for legibility only.
    ...
    if (params->debugEnabled) {
        ...
        shouldDebug = isDebugGray && (
            (fabs(inputFloat[0] - 0.000) < 0.001) ||
            (fabs(inputFloat[0] - 0.250) < 0.005) ||
            (fabs(inputFloat[0] - 0.500) < 0.005) ||
            (fabs(inputFloat[0] - 0.750) < 0.005)
        );
        if (shouldDebug) {
    ...
    //
    // Snap near-neutral decodes to the neutral axis. The CLUT sampler grid
    // steps uniformly across 0..0xFFFF, so Lab input encoded as Lab16 lands
    // at grid offsets that miss the true neutral midpoint (`a = b = 0` lives
    // at `0x8080`, grid steps at `0x8000`, `0x8800`, ...). The decoded Lab
    // at those midpoint cells is `a = b ≈ -0.498` — chromatic enough that
    // the downstream chroma-preservation stage leaks CMY out of an
    // otherwise-neutral input. The snap is well below any meaningful chroma
    // and only affects cells that were already within one grid step of the
    // neutral axis.
    const cmsFloat64Number NEUTRAL_AXIS_SNAP = 0.6;
    cmsCIELab inputLab;
    inputLab.L = (cmsFloat64Number)labFloat[0];
    inputLab.a = (fabs((cmsFloat64Number)labFloat[1]) < NEUTRAL_AXIS_SNAP) ? 0.0 : (cmsFloat64Number)labFloat[1];
    inputLab.b = (fabs((cmsFloat64Number)labFloat[2]) < NEUTRAL_AXIS_SNAP) ? 0.0 : (cmsFloat64Number)labFloat[2];
    ...
    if (shouldDebug) {
        ...
        fprintf(stderr, "[C-DEBUG] Stage 3 - BPC Scale: %.6f\n", params->kOnlyBlackpointCompensationScale);
    }

    // Stage 3: Apply K-Only BPC to Lab (line 1918)
    cmsCIELab scaledLab;
    ApplyKOnlyBlackpointCompensation(&inputLab, &scaledLab, params->kOnlyBlackpointCompensationScale, params->debugEnabled);
    ...
    const cmsFloat64Number L_TOLERANCE = 0.125;
    const cmsFloat64Number K_ADJUSTMENT_STEP = 0.125 / 4; // Adjust K by 0.5% each iteration
    ...
    outputCMYK[3] = fmin(K_MAX, normalizedCMYK[3] + normalizedGrayComponent / 2.0); // K
    ...
    while (iterationCount++ < MAX_ITERATIONS) {
        ...
        if (fabs(deltaL) <= L_TOLERANCE) break;
        ...
        outputCMYK[3] += (outputLab.L > targetL) ? -K_ADJUSTMENT_STEP : K_ADJUSTMENT_STEP;
    ...
    if (inputChroma > 1.0 && outputChroma > 1.0 && (inputChroma - outputChroma) > 0.5) {
        ...
        const int MAX_CHROMA_ITERATIONS = MAX_ITERATIONS;  // Updated to match JS (transform.js line 949)
        ...
        while (chromaIterations++ < MAX_CHROMA_ITERATIONS && primaryStep != 0.0) {
            ...
            optimizedCMYKFloat[3] = fmin(K_MAX, (cmsFloat32Number)optimizedCMYK[3]);
            ...
            while (kAdjustIterations++ < MAX_ITERATIONS) {
                ...
                previousDeltaL = deltaL;
                optimizedCMYK[3] += (optimizedLab.L > targetL) ? -K_ADJUSTMENT_STEP : K_ADJUSTMENT_STEP;
    ...
    const int MAX_FINAL_ITERATIONS = MAX_ITERATIONS;  // Much higher limit since JS has no limit
    const cmsFloat64Number FINAL_L_TOLERANCE = 0.125;
    const cmsFloat64Number K_ADJUSTMENT_STEP_FINAL = 0.125 / 2; // Adjust K by 0.5% each iteration
    ...
    while (fabs(finalLab.L - finalTargetL) > FINAL_L_TOLERANCE
        ...
        cmsFloat64Number stepSize = (fabs(deltaL) / 2.0 < K_ADJUSTMENT_STEP_FINAL) ? fabs(deltaL) / 2.0 : K_ADJUSTMENT_STEP_FINAL;
    ...
    cmsUInt32Number dwFlags)
{
    ...
    //
    // `samplerInputType` is the 16-bit encoding that matches the profile's
    // input colorspace. The sampler receives `cmsUInt16Number In[]` from
    // `cmsStageSampleCLut16bit` and feeds it directly to `input2lab`, so
    // LittleCMS handles the 16-bit → float decode with the correct encoding
    // for each colorspace (notably Lab16 where `a = 0` is `0x8080`, not `0x0000`).
    cmsUInt32Number samplerInputType;
    if (inputChannels == 1) {
        samplerInputType = TYPE_GRAY_16;
    } else if (inputChannels == 3) {
        if (inputColorSpace == cmsSigRgbData) {
            samplerInputType = TYPE_RGB_16;
        } else {
            samplerInputType = TYPE_Lab_16;
        }
    } else {
        samplerInputType = TYPE_CMYK_16;
    }
    ...
    cmsHTRANSFORM input2lab = cmsCreateTransformTHR(
        ContextID,
        hInputProfile, samplerInputType,
        hLabProfile, TYPE_Lab_FLT,
        internalIntent,
        cmsFLAGS_BLACKPOINTCOMPENSATION | dwFlagsApplyBPC
    );
    ...
    cmsFloat64Number kOnlyBlackpointCompensationScale = ComputeKOnlyBlackpointCompensationScale(ContextID, hOutputProfile, INTENT_RELATIVE_COLORIMETRIC, &kOnlyBlackpointLab);
    
    #if K_ONLY_PARITY_DEBUGGING
    fprintf(stderr, "\n[C] K-Only BPC Scale: %.6f\n", kOnlyBlackpointCompensationScale);
    ...
    // 7. Detect profiles that don't require K-Only GCR
    //
    // A profile natively enforces K-only behavior when BOTH conditions are true:
    //   1. CMYK(100,100,100,100) roundtrips to CMYK(0,0,0,100) — black maps to pure K
    //   2. K-only roundtrip preserves K across the tonal range — for each K in
    //      {100,95,90,80,70,60,50,40,30,20,10,5}, CMYK(0,0,0,K) → Lab → CMYK(0,0,0,K±2)
    //
    // When both conditions hold, K-Only GCR processing is redundant. We fall back
    // to Relative Colorimetric via _cmsDefaultICCintents.
    cmsBool doesNotRequireKOnlyGCR = FALSE;
    {
        // Condition 1: CMYK(100,100,100,100) → Lab → CMYK produces (0,0,0,100)
        // TYPE_CMYK_FLT uses 0-100 percentage range (not 0-1)
        cmsFloat32Number fullBlackCMYK[4] = { 100.0f, 100.0f, 100.0f, 100.0f };
        cmsFloat32Number roundtripLab[3];
        cmsDoTransform(cmyk2lab, fullBlackCMYK, roundtripLab, 1);

        cmsFloat32Number roundtripCMYK[4];
        cmsDoTransform(lab2cmyk, roundtripLab, roundtripCMYK, 1);

        // TYPE_CMYK_FLT already returns 0-100 range — no scaling needed
        cmsFloat64Number roundtripC = (cmsFloat64Number)roundtripCMYK[0];
        cmsFloat64Number roundtripM = (cmsFloat64Number)roundtripCMYK[1];
        cmsFloat64Number roundtripY = (cmsFloat64Number)roundtripCMYK[2];
        cmsFloat64Number roundtripK = (cmsFloat64Number)roundtripCMYK[3];

        cmsFloat64Number cmySum = roundtripC + roundtripM + roundtripY;
        cmsBool condition1 = (cmySum < 0.000001 && roundtripK > 99.99999);

        if (condition1) {
            // Condition 2: K-only roundtrip preserves K across the tonal range
            static const cmsFloat64Number kValues[] = { 100, 95, 90, 80, 70, 60, 50, 40, 30, 20, 10, 5 };
            static const cmsUInt32Number kValueCount = 12;
            cmsBool condition2 = TRUE;

            for (cmsUInt32Number kIndex = 0; kIndex < kValueCount; kIndex++) {
                cmsFloat64Number kPercent = kValues[kIndex];
                // TYPE_CMYK_FLT uses 0-100 percentage range
                cmsFloat32Number kOnlyCMYK[4] = { 0.0f, 0.0f, 0.0f, (cmsFloat32Number)kPercent };
                cmsFloat32Number kOnlyLab[3];
                cmsDoTransform(cmyk2lab, kOnlyCMYK, kOnlyLab, 1);

                cmsFloat32Number kOnlyResult[4];
                cmsDoTransform(lab2cmyk, kOnlyLab, kOnlyResult, 1);

                // TYPE_CMYK_FLT already returns 0-100 range — no scaling needed
                cmsFloat64Number resultC = (cmsFloat64Number)kOnlyResult[0];
                cmsFloat64Number resultM = (cmsFloat64Number)kOnlyResult[1];
                cmsFloat64Number resultY = (cmsFloat64Number)kOnlyResult[2];
                cmsFloat64Number resultK = (cmsFloat64Number)kOnlyResult[3];

                // C+M+Y must be ≈ 0 (< 1% total) and K must be within ±2 of input K
                // Lab→CMYK roundtrip introduces tiny CMY residuals (typically < 0.03%)
                // due to float precision in profile interpolation
                cmsFloat64Number resultCMYSum = resultC + resultM + resultY;
                if (resultCMYSum >= 1.0 || fabs(resultK - kPercent) > 2.0) {
                    condition2 = FALSE;
                    break;
                }
            }

            doesNotRequireKOnlyGCR = condition2;
        }
    }

    // Early return: fall back to Relative Colorimetric via _cmsDefaultICCintents
    if (doesNotRequireKOnlyGCR) {
        cmsDeleteTransform(input2lab);
        cmsDeleteTransform(lab2cmyk);
        cmsDeleteTransform(cmyk2lab);
        cmsCloseProfile(hLabProfile);

        cmsUInt32Number relativeColorimetricIntents[2] = {
            INTENT_RELATIVE_COLORIMETRIC,
            INTENT_RELATIVE_COLORIMETRIC
        };
        return _cmsDefaultICCintents(
            ContextID, nProfiles, relativeColorimetricIntents,
            hProfiles, BPC, AdaptationStates, dwFlags
        );
    }
    ...
    params->kOnlyBlackpointCompensationScale = kOnlyBlackpointCompensationScale;
    params->scaledKOnlyBlackpointLab = scaledKOnlyBlackpointLab;
    params->neutralTolerance = 2.0;
    params->doesNotRequireKOnlyGCR = doesNotRequireKOnlyGCR;
    params->debugEnabled = (dwFlags & cmsFLAGS_DEBUG_COLOR_ENGINE) ? TRUE : FALSE;
    ...
    if (inputChannels == 1) {
        ...
        if (clutStage) {
            ...
            PatchKOnlyNeutralApproach(clutStage, gridPoints);
        }
    }
...
// Non-static: referenced by color-engine-plugin.c for chaining via Next pointer
cmsPluginRenderingIntent KOnlyGCRIntentPlugin = {
    .base = {
