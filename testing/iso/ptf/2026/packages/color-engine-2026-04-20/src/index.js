 ...
 * the API surface is preserved as the extension point for future
 * refinements. A freshly-created engine starts in this default state.
 *
 * K-only refinements shipped in 2026-04-20 that always run (no opt-in):
 *
 * - Scale-degeneracy correction in
 *   `ComputeKOnlyBlackpointCompensationScale` — substitutes the profile's
 *   raw `Y(K = 100 %)` into the numerator when the BPC-flattened
 *   `yBlack ≈ yKOnly ≈ 0` collapses the standard formula to `1.0`.
 * - Post-sampling neutral-diagonal patch (`PatchKOnlyNeutralApproach`)
 *   enforcing corner saturation + `K[1,1,1]` cap + monotonic diagonal.
 * - Lab-input sampler decode fix — internal `input2lab` transform is built
 *   with 16-bit input encoding matching the profile colorspace.
 * - Lab neutral-axis snap in the sampler — decoded `|a|, |b| < 0.6` snap
 *   to `0.0` to cancel CLUT grid aliasing near the neutral axis.
 * - Sampler iteration-constant refinements (finer K/CMY tolerances and
 *   step sizes; chroma iteration budget restored to full).
 *
 * Future parameters are added as `@property` entries here and as
 * validated branches inside {@link ColorEngine#setParameters}.
 *
 * @typedef {Object} EngineParameters
 */

/**
 * Partial input shape for {@link ColorEngine#setParameters}.
 *
 * - An **omitted** key or `undefined` value leaves that parameter unchanged.
 * - A `null` value resets that parameter to its default.
 * - Any other legal value sets that parameter (validated).
 *
 * `setParameters(null)` is equivalent to calling `setParameters` with
 * every known key set to `null`, i.e. reset everything.
 *
 * @typedef {Object} EngineParametersInput
 */

/**
 * Build the default parameters object.
 * @returns {EngineParameters}
 */
function defaultEngineParameters() {
  return {};
}
...
export class ColorEngine {
  ...
  /**
   * Return a new object representing this engine's current parameters.
   * Mutating the returned object has no effect on the engine — call
   * {@link ColorEngine#setParameters} to change state.
   *
   * @returns {EngineParameters}
   */
  getParameters() {
  ...
  /**
   * Update this engine's parameters.
   *
   * - `setParameters(null)` resets **every** parameter to its default.
   * - `setParameters(partial)` merges `partial` into the current state:
   *   - `undefined` / missing key → that parameter is unchanged.
   *   - `null` → that parameter resets to its default.
   *   - Legal value → that parameter is set (validated).
   *
   * @param {EngineParametersInput | null} partial
   * @returns {void}
   * @throws {TypeError} If `partial` is not `null`, `undefined`, or a plain object.
   * @throws {TypeError} If a parameter is set to a value that is not `null`,
   *   `undefined`, or a value of its declared type.
   */
  setParameters(partial) {
    if (partial === null) {
      this.#parameters = defaultEngineParameters();
      this.#pushParametersToWASM();
      return;
    }
    if (partial === undefined) return;
    if (typeof partial !== 'object') {
      throw new TypeError(`setParameters: expected object, null, or undefined; got ${typeof partial}`);
    }

    // No parameters defined in the 2026-04-20 drop-in. Future parameter
    // handling goes here, validated field-by-field. The merge is still
    // performed so that any `in` check on future fields is safe.
    /** @type {EngineParameters} */
  ...
  /**
   * Push this engine's current parameters into the WASM module. Called
   * from {@link ColorEngine#setParameters} after validation.
   * @returns {void}
   */
...
export const VERSION = '2026-04-20';
