...
static cmsHTRANSFORM CreateKOnlyGCRMultiprofileTransform(
    ...
    if (nProfiles == 2) {
        ...
        // Signal the Color Engine Plugin to:
        // 1. Promote Lab16 input to float for pipeline evaluation (avoids 16-bit artifacts)
        // 2. Skip pipeline optimization (prevents merging analytical stages into CLUT)
        cmsUInt32Number inputFormatCopy = InputFormat;
        cmsUInt32Number outputFormatCopy = OutputFormat;
        cmsUInt32Number dwFlagsCopy = dwFlags | cmsFLAGS_LAB16_FLOAT_PROMOTION;
    ...
    // 4. Create transform from concatenated pipeline
    //    Signal Lab16 float promotion + skip optimization
    {
        cmsUInt32Number inputFormatCopy = InputFormat;
        cmsUInt32Number outputFormatCopy = OutputFormat;
        cmsUInt32Number dwFlagsCopy = dwFlags | cmsFLAGS_LAB16_FLOAT_PROMOTION;
