...
cmsPipeline* CreateCompositeLUTTransform(
    ...
    // Set output color space for achromatic coercion in sampler
...
cmsHTRANSFORM CreateMultiprofileTransform(
    ...
    // For 2-profile chains, use native transform (always works)
    if (nProfiles == 2) {
    ...
    // For 3+ profile chains, build composite CLUT via chained 2-profile transforms
