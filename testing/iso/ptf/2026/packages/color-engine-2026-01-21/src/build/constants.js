...
export const { macros, constants, code } = await (async () => {
    ...
    const FunctionMatcher = /^#define\s+(?<identifier>[A-Z]+\d*_SH)(?<arguments>\([a-z]+\))\s+(?<macro>\(.+?\))\s*;?\s*(?:\/\/|$)/mg;
    ...
    for (const { groups } of FunctionMatcher[Symbol.matchAll](lcms2Header)) {
        ...
        code.push(`const ${groups.identifier} = ${groups.arguments} => ${groups.macro};`);
    }

    const ConstantMatcher = /^#define\s+(?<identifier>PT_[A-Z][A-Za-z0-9]+|TYPE_[A-Z][A-Za-z0-9_]+)\s+(?<macro>.+?)\s*;?\s*(?:\/\/|$)/mg;
    ...
    for (const { groups } of ConstantMatcher[Symbol.matchAll](lcms2Header)) {
        ...
        code.push(`const ${groups.identifier} = ${groups.macro}; // ${constants[groups.identifier]}`);
    }
