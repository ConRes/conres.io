...
static void ApplyStandardRGBBPCScalingInXYZ(const cmsCIEXYZ* inputXYZ, cmsCIEXYZ* outputXYZ, cmsFloat64Number scale);
...
cmsInt32Number CompositeLUTSamplerFloat(
    ...
    if (
        // cargo->applyBPC && fabs(cargo->bpcScale - 1.0) > 1e-7 &&
        ...
        // if (isBlackInput) fprintf(stderr, "[BPC] Before: RGB=[%.6f,%.6f,%.6f] XYZ=[%.6f,%.6f,%.6f] scale=%.6f\n", output[0], output[1], output[2], outputXYZ.X, outputXYZ.Y, outputXYZ.Z, cargo->bpcScale);

        // Apply BPC scaling in XYZ space with flattening
        ApplyStandardRGBBPCScalingInXYZ(&outputXYZ, &scaledXYZ, cargo->bpcScale);
 ...
 *
 * Used when cmsFLAGS_MULTIPROFILE_BPC_SCALING is set to create transforms
 ...
 *
 * Algorithm (from k-only-gcr.c ApplyKOnlyBPC):
 ...
 */
static void ApplyStandardRGBBPCScalingInXYZ(
 ...
 * higher precision during LUT sampling. Called when cmsFLAGS_MULTIPROFILE_BPC_SCALING
 * is set.
 *
...
static cmsBool BuildChainedTransformsFloat(
    ...
    for (i = 0; i < nTransforms; i++) {
        ...
        // Create 2-profile float transform with original flags (including BPC)
        transforms[i] = cmsCreateTransformTHR(
            ContextID,
            inputProfile, inputFormat,
            outputProfile, outputFormat,
    ...
    cargo->bpcScale = 1.0;
    cargo->applyBPC = (dwFlags & cmsFLAGS_BLACKPOINTCOMPENSATION) != 0;
    ...
    if (cargo->applyBPC) {
        ...
        if (outputCS == cmsSigRgbData) {
            ...
            if (hXYZProfile != NULL) {
                // cargo->outputToSRGB = cmsCreateTransformTHR(
                //     ContextID,
                        ...
                        if (hXYZProfile2 != NULL) {
                            ...
                            if (outputToXYZWithBPC != NULL) {
                                ...
                                // Compute scale = (1 - yWeWant) / (1 - yWeGet)
                                {
                                    ...
                                    cargo->bpcScale = (1.0 - yWeWant) / (1.0 - yWeGet);

                                    // fprintf(stderr, "[Scale Debug] Scale = (1 - %.6f) / (1 - %.6f) = %.6f\n", yWeWant, yWeGet, cargo->bpcScale);
                                }
                            } else {
...
cmsPipeline* CreateCompositeLUTTransform(
    ...
    // Validate parameters
    // Accept 2+ profiles (2-profile supported when cmsFLAGS_MULTIPROFILE_BPC_SCALING is set)
    ...
    useFloat = (dwFlags & cmsFLAGS_MULTIPROFILE_BPC_SCALING) != 0;
...
cmsHTRANSFORM CreateMultiprofileTransform(
    ...
    if (nProfiles == 2 && !(dwFlags & cmsFLAGS_MULTIPROFILE_BPC_SCALING)) {
...
/**
 * Applies achromatic coercion to Lab output values
 *
 * Colorimetric truth: Pure black (L=0%) and pure white (L=100%) are achromatic
 * points with zero chroma. This function ensures a* and b* are neutral (0)
 * at these L extremes.
 *
 * This is NOT just a cache workaround - it's a colorimetric correction that
 * ensures physically correct output for achromatic extremes.
 *
 * @param output        Output buffer (will be modified in place)
 * @param pixelCount    Number of pixels in buffer
 * @param outputFormat  LittleCMS output format (TYPE_Lab_*)
 */
static void CoerceAchromaticLab(
    void* output,
    cmsUInt32Number pixelCount,
    cmsUInt32Number outputFormat
) {
    cmsUInt32Number i;
    cmsUInt32Number isLab;
    cmsUInt32Number bytesPerChannel;
    cmsUInt32Number isFloat;

    // Check if output is Lab format
    // PT_Lab = 10, extracted via T_COLORSPACE macro
    isLab = (T_COLORSPACE(outputFormat) == PT_Lab);
    if (!isLab) return;

    // Get bytes per channel and float flag
    bytesPerChannel = T_BYTES(outputFormat);
    isFloat = T_FLOAT(outputFormat);

    if (isFloat) {
        // Float Lab: L=0.0 or L=100.0 → a=0.0, b=0.0
        if (bytesPerChannel == 4) {
            // Float32
            cmsFloat32Number* lab = (cmsFloat32Number*)output;
            for (i = 0; i < pixelCount; i++) {
                cmsFloat32Number L = lab[i * 3];
                if (L <= 0.0f || L >= 100.0f) {
                    lab[i * 3 + 1] = 0.0f;  // a* neutral
                    lab[i * 3 + 2] = 0.0f;  // b* neutral
                }
            }
        } else if (bytesPerChannel == 0) {
            // Float64 (bytes=0 indicates double in lcms2 format)
            cmsFloat64Number* lab = (cmsFloat64Number*)output;
            for (i = 0; i < pixelCount; i++) {
                cmsFloat64Number L = lab[i * 3];
                if (L <= 0.0 || L >= 100.0) {
                    lab[i * 3 + 1] = 0.0;  // a* neutral
                    lab[i * 3 + 2] = 0.0;  // b* neutral
                }
            }
        }
    } else {
        ...
        if (bytesPerChannel == 2) {
            // Lab16: L=0 or L=65535 → a=32896, b=32896
            cmsUInt16Number* lab = (cmsUInt16Number*)output;
            for (i = 0; i < pixelCount; i++) {
                cmsUInt16Number L = lab[i * 3];
                if (L == 0 || L == 65535) {
                    lab[i * 3 + 1] = 32896;  // a* neutral in Lab16
                    lab[i * 3 + 2] = 32896;  // b* neutral in Lab16
                }
            }
        } else if (bytesPerChannel == 1) {
            // Lab8: L=0 or L=255 → a=128, b=128
            cmsUInt8Number* lab = (cmsUInt8Number*)output;
            for (i = 0; i < pixelCount; i++) {
                cmsUInt8Number L = lab[i * 3];
                if (L == 0 || L == 255) {
                    lab[i * 3 + 1] = 128;  // a* neutral in Lab8
                    lab[i * 3 + 2] = 128;  // b* neutral in Lab8
                }
            }
        }
    }
 ...
 *
 * This function performs a color transform and then applies achromatic
 * coercion for Lab output: when L=0% (black) or L=100% (white), it forces
 * a* and b* to their neutral values (0 in encoded space).
 *
 * Use this for Gray → Lab transforms where the uninitialized cache bug
 * would otherwise cause incorrect a/b values at achromatic extremes.
 *
...
void DoTransformWithAchromaticCoercion(
    ...
    // Apply achromatic coercion if output is Lab
    CoerceAchromaticLab(OutputBuffer, Size, OutputFormat);
}
