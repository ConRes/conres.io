 ...
 */
#define cmsFLAGS_MULTIPROFILE_BPC_SCALING 0x20000000
 ...
 * Used when cmsFLAGS_MULTIPROFILE_BPC_SCALING is set.
 *
...
typedef struct {
    ...
    cmsFloat64Number bpcScale;           // BPC scale factor computed from lifted black
    cmsBool applyBPC;                    // Whether to apply explicit BPC scaling
    cmsHTRANSFORM inputToXYZ;            // Helper: input color space → XYZ (for BPC at input)
    cmsHTRANSFORM xyzToInput;            // Helper: XYZ → input color space (for BPC at input)
    cmsHTRANSFORM outputToSRGB;           // Helper: output color space → XYZ (for BPC)
    cmsHTRANSFORM outputToXYZ;           // Helper: output color space → XYZ (for BPC)
    cmsHTRANSFORM xyzToOutput;           // Helper: XYZ → output color space (for BPC)
 ...
 * complex multiprofile chains when cmsFLAGS_MULTIPROFILE_BPC_SCALING is set.
 *
 * @param In     Input grid coordinates (16-bit quantized)
 * @param Out    Output values to write (16-bit)
