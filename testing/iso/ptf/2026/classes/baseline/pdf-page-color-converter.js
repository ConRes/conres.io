...
export class PDFPageColorConverter extends CompositeColorConverter {
    ...
    #extractImageInput(imageData) {
        ...
        if (colorSpaceInfo.type === 'Indexed') {
            ...
            return {
                ...
                pdfColorSpaceType: /** @type {import('./pdf-image-color-converter.js').PDFColorSpaceType} */ (colorSpaceInfo.type),
                bitsPerComponent,
    ...
    async #convertIndexedImage(imageData, imageInput, context) {
        ...
        // The lookup table bytes are already in 8-bit encoding matching LittleCMS TYPE_*_8
        // formats: RGB 0-255, Gray 0-255, Lab L=0-255→0-100 a/b=0-255→-128..+127.
        // Each palette entry is treated as one "pixel" for the buffer transform.
        const inputBuffer = lookupData.slice(0, expectedBytes);
        const inputColorSpace = /** @type {import('./color-converter.js').ColorType} */ (colorSpace);
        const outputColorSpace = config.destinationColorSpace;
        const outputChannels = outputColorSpace === 'CMYK' ? 4 : outputColorSpace === 'RGB' ? 3 : outputColorSpace === 'Gray' ? 1 : 3;

        // Determine source profile: Lab uses built-in 'Lab', others use ICC data
        const effectiveSourceProfile = /** @type {ArrayBuffer | 'Lab'} */ (
            sourceProfile
            ?? (colorSpace === 'Lab' ? 'Lab' : undefined)
            ?? (colorSpace === 'RGB' ? config.sourceRGBProfile : undefined)
            ?? (colorSpace === 'Gray' ? config.sourceGrayProfile : undefined)
        );
        ...
        try {
            // convertColorsBuffer respects config.intermediateProfiles for multiprofile transforms
            const result = await this.convertColorsBuffer(inputBuffer, {
                ...
                outputBitsPerComponent: /** @type {import('./color-conversion-policy.js').BitDepth} */ (8),
            });

            if (result.outputPixels && result.pixelCount > 0) {
                // outputPixels is the converted lookup table (outputChannels bytes per color)
                const cmykLookup = result.outputPixels instanceof Uint8Array
                    ? result.outputPixels
                ...
                let hexStr = '';
                const lookupLength = numColors * outputChannels;
                for (let i = 0; i < lookupLength && i < cmykLookup.length; i++) {
                    hexStr += cmykLookup[i].toString(16).padStart(2, '0');
                }
                const newLookupPDF = PDFHexString.of(hexStr);

                // Map output color space to PDF device color space name
                const pdfDeviceColorSpace = outputColorSpace === 'CMYK' ? 'DeviceCMYK'
                    : outputColorSpace === 'RGB' ? 'DeviceRGB'
                    : outputColorSpace === 'Gray' ? 'DeviceGray'
                    : 'DeviceCMYK';

                // Create new ColorSpace array: [/Indexed /Device* hival newLookup]
                const newColorSpaceArray = pdfContext.obj([
                    PDFName.of('Indexed'),
                    PDFName.of(pdfDeviceColorSpace),
