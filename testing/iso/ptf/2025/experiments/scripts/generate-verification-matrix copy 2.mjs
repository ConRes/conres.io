...
// Import color operator regex from refactored classes
    ...
    for (let pageIdx = 0; pageIdx < pages.length; pageIdx++) {
        ...
        for (let streamIdx = 0; streamIdx < streamRefs.length; streamIdx++) {
            ...
            try {
                ...
                // Use the original regex for consistent operatorIndex counting
                ...
                for (const match of regexMatches) {
                    ...
                    // Track colorspace state from CS/cs operators (don't count as color operations)
                    if (groups.csOp && groups.name) {
                        const csName = groups.name.replace(/^\//, '');
                        if (groups.csOp === 'CS') {
                            currentStrokeColorSpace = csName;
                        } else {
                            currentFillColorSpace = csName;
                        }
                        continue;
                    }
                    ...
                    // Gray color (G/g)
                        ...
                        const grayValue = parseFloat(groups.gray.trim());
                        matches.push({
                            pageNum,
                            streamIndex: streamIdx,
                            operatorIndex: operatorIndex++,
                    ...
                    // SC/sc/SCN/scn operators - use tracked colorspace
                    else if (groups.n && groups.scOp) {
                        const scnValues = groups.n.trim().split(/\s+/).map(parseFloat);
                        const operator = groups.scOp;
                        const isStroke = operator === 'SC' || operator === 'SCN';
                        const csName = isStroke ? currentStrokeColorSpace : currentFillColorSpace;

                        // Look up actual colorspace type from definitions
                        let colorspace;
                        if (csName && colorSpaceDefinitions[csName]) {
                            colorspace = getDisplayColorspace(colorSpaceDefinitions[csName].colorSpaceType);
                        } else {
                            // Fallback: infer from value count (same as original behavior)
                            if (scnValues.length === 1) colorspace = 'ICCBasedGray';
                            else if (scnValues.length === 3) colorspace = 'ICCBasedRGB';
                            else if (scnValues.length === 4) colorspace = 'ICCBasedCMYK';
