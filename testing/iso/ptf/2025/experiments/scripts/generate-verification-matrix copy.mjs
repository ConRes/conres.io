...
// Import PDFContentStreamColorConverter for proper colorspace tracking
import { PDFContentStreamColorConverter } from '../../classes/pdf-content-stream-color-converter.js';
...
function runConversion(args, logFile) {
    return new Promise((resolve, reject) => {
        ...
        logStream.write(`node ${[path.relative(experimentsDir, args[0]), ...args.slice(1)].join(' ')}\n\n`);
...
function parseComparisonOutput(output) {
    ...
    // Parse individual images: "Im0: MATCH 3812x2750 DeviceCMYK (∆E avg=0.75 max=4.96 (98% ≤3))"
 ...
 * @param {string} colorSpaceType - Internal colorspace type (sRGB, sGray, Lab, CMYK)
 * @param {string} operator - PDF operator used (RG, rg, K, k, G, g, SC, sc, etc.)
 * @returns {string} Display colorspace name
 */
function getDisplayColorspace(colorSpaceType, operator) {
    // Device* operators always indicate Device colorspaces
    if (operator === 'RG' || operator === 'rg') return 'DeviceRGB';
    if (operator === 'K' || operator === 'k') return 'DeviceCMYK';
    if (operator === 'G' || operator === 'g') return 'DeviceGray';

    // SC/sc/SCN/scn operators use the tracked colorspace
 ...
 * Uses PDFContentStreamColorConverter.parseContentStream() for proper colorspace tracking.
 *
...
async function extractColorsFromPDF(pdfPath) {
    ...
    // Create a minimal converter instance just for parsing (no conversion needed)
    // We only use the static parseContentStream method pattern
    const converterForParsing = new PDFContentStreamColorConverter({
        renderingIntent: 'relative-colorimetric',
        blackPointCompensation: false,
        useAdaptiveBPCClamping: false,
        destinationColorSpace: 'CMYK',
        // @ts-ignore - destinationProfile not needed for parsing only
    ...
    for (let pageIdx = 0; pageIdx < pages.length; pageIdx++) {
        ...
        /** @type {import('../../classes/pdf-content-stream-color-converter.js').ColorSpaceState} */
        ...
        for (let streamIdx = 0; streamIdx < streamRefs.length; streamIdx++) {
            ...
            try {
                ...
                // Use parseContentStream for proper colorspace tracking
                const parseResult = converterForParsing.parseContentStream(text, currentColorSpaceState);
                const { operations, finalState } = parseResult;

                // Update colorspace state for next stream
                currentColorSpaceState = finalState;

                let operatorIndex = 0;
                for (const op of operations) {
                    // Skip colorspace-only operations (CS/cs)
                    if (op.type === 'colorspace' && !op.values) {
                        continue;
                    }

                    // Determine the actual colorspace based on operator type and tracked state
                    let colorspace;
                    if (op.type === 'rgb') {
                        ...
                        // Look up the actual colorspace type from definitions
                        const csDef = colorSpaceDefinitions[op.colorSpaceName];
                        colorspace = getDisplayColorspace(csDef?.colorSpaceType || 'Unknown', op.operator || '');
                    } else {
...
function findMatchingInputColors(inputColors, inputSpec) {
    ...
    return inputColors.filter(color => {
        ...
        // Colorspace type must be compatible with operator
        switch (inputSpec.colorspace) {
            case 'ICCBasedRGB':
                // ICCBased RGB uses SCN/scn operators with 3 values, or RG/rg
                return (color.operator === 'SCN' || color.operator === 'scn' ||
                        color.operator === 'SC' || color.operator === 'sc' ||
                        color.operator === 'RG' || color.operator === 'rg') &&
                       color.values.length === 3;
            case 'DeviceRGB':
                // Device RGB uses RG/rg operators
                return color.operator === 'RG' || color.operator === 'rg';
            case 'ICCBasedGray':
                // ICCBased Gray uses SCN/scn operators with 1 value, or G/g
                return (color.operator === 'SCN' || color.operator === 'scn' ||
                        color.operator === 'SC' || color.operator === 'sc' ||
                        color.operator === 'G' || color.operator === 'g') &&
                       color.values.length === 1;
            case 'DeviceGray':
                // Device Gray uses G/g operators
                return color.operator === 'G' || color.operator === 'g';
            case 'ICCBasedCMYK':
                // ICCBased CMYK uses SCN/scn operators with 4 values, or K/k
                return (color.operator === 'SCN' || color.operator === 'scn' ||
                        color.operator === 'SC' || color.operator === 'sc' ||
                        color.operator === 'K' || color.operator === 'k') &&
                       color.values.length === 4;
            case 'DeviceCMYK':
                // Device CMYK uses K/k operators
                return color.operator === 'K' || color.operator === 'k';
            case 'Lab':
                // Lab uses SCN/scn operators with 3 values (L, a, b)
        ...
        for (const [inputName, input] of Object.entries(jobs.inputs)) {
            for (const [outputName, output] of Object.entries(jobs.outputs)) {
                for (const [configName, config] of Object.entries(jobs.configurations)) {
                    ...
                    // ICC profile (already resolved to absolute path)
