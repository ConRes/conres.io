...
const COLOR_ENGINE_POLICIES = await (async () => {
        ...
        if (key === 'intermediateProfiles' && Array.isArray(value)) {
            ...
            for (const profileSpecifier of value) {
                if (!/^\.{0,2}\//.test(profileSpecifier)) {
                    ...
                    processedIntermediateProfiles.push(relativeProfilePath);
                    resolveRelativePaths[profileSpecifier] = relativeProfilePath;
                }
            }
    ...
    // console.dir({ importedRules, processedRules, resolveRelativePaths });
