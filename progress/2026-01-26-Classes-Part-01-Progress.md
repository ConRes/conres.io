...
# Step 2: Run full verification matrix (24 conversions + 12 comparisons)
...
# Step 3: Verify all comparisons passed
# Expected: "Status: ALL COMPARISONS PASSED" in SUMMARY.md
# Expected: Exit code 0
```

**Configuration File:**

- `testing/iso/ptf/2025/experiments/configurations/2026-01-26-CLASSES-001.json`

**Verification Matrix (24 conversions):**

| Dimension          | Values                                                                                    |
| ------------------ | ----------------------------------------------------------------------------------------- |
| Inputs (2)         | F-01 Fixtures, Type Sizes and Lissajou                                                    |
| Outputs (3)        | eciCMYK v2 - Relative Colorimetric, eciCMYK v2 - K-Only GCR, FIPS_WIDE - Rel Colorimetric |
| Configurations (4) | Refactored Main, Legacy Main, Refactored Workers, Legacy Workers                          |

**Comparisons (12 pairs):**

- For each (input × output × modality): Legacy vs Refactored
- Uses `compare-pdf-color.js` for structural comparison (ICC profiles, images, content streams)

**Baseline Output Files:**

```
output/2026-01-26-001/
├── SUMMARY.json                    # Structured results with comparison outcomes
├── SUMMARY.md                      # Markdown summary with pass/fail status
...
# Step 3: Run full verification matrix (comparisons MUST pass)
node testing/iso/ptf/2025/experiments/scripts/generate-verification-matrix.mjs \
  --config=testing/iso/ptf/2025/experiments/configurations/2026-01-26-CLASSES-001.json

# Step 4: Verify all comparisons passed
# Expected: "Status: ALL COMPARISONS PASSED" in SUMMARY.md
# Expected: Exit code 0
```
...
| Verification matrix      | Exit code 0                               | Investigate diff, rollback |
| Comparisons (12 pairs)   | All pass (Legacy vs Refactored identical) | Investigate diff, rollback |
...
#### Step 2: Full Verification Matrix with Comparisons (REQUIRED)

```bash
cd ~/Projects/conres/conres.io

# Run full verification matrix (24 conversions + 12 comparisons)
node testing/iso/ptf/2025/experiments/scripts/generate-verification-matrix.mjs \
  --config=testing/iso/ptf/2025/experiments/configurations/2026-01-26-CLASSES-001.json
```
...
1. All 24 conversions complete successfully
2. All 12 comparisons pass (Legacy vs Refactored for each input × output × modality)
3. `SUMMARY.md` shows "Status: ALL COMPARISONS PASSED"
4. Script exits with code 0

**What `compare-pdf-color.js` Checks:**

| Check            | Description                                           | Tolerance        |
| ---------------- | ----------------------------------------------------- | ---------------- |
| ICC Profile Hash | Output intent profile must match                      | Exact            |
| Image Dimensions | All images must have same dimensions                  | Exact            |
| Image BPC        | Bits per component must match                         | Exact            |
| Image ColorSpace | Color space type must match                           | Exact            |
| Image Pixels     | Sampled pixels compared with tolerance                | ΔE ≤ 2.0         |
| Content Streams  | Operator counts and structure hash                    | Exact            |
...
#### Step 3: Compare to Historical Baselines (Performance Only)

After running the verification matrix, compare timing results to 2026-01-23-001:
...
| All comparisons passed         | N/A            | 12/12          | Must be 12/12 |
...
- [ ] `yarn test` passes
- [ ] Baseline verification matrix saved to `output/2026-01-26-001/`
...
- [ ] Verification matrix comparisons pass (exit code 0)
- [ ] Output saved to `output/2026-01-26-XXX/` (auto-incremented)

#### Final Verification

- [ ] Full test suite passes (`yarn test`)
- [ ] Verification matrix complete (24 conversions)
- [ ] All 12 comparisons pass (Legacy vs Refactored)
- [ ] `SUMMARY.md` shows "Status: ALL COMPARISONS PASSED"
...
git stash apply "stash^{/2026-01-26-stage-1-PDFImageColorConverter}"
git diff testing/iso/ptf/2025/classes/pdf-image-color-converter.js
git stash drop "stash^{/2026-01-26-stage-1-PDFImageColorConverter}"
...
git stash apply "stash^{/2026-01-26-stage-2-PDFContentStreamColorConverter}"
git stash drop "stash^{/2026-01-26-stage-2-PDFContentStreamColorConverter}"
...
git stash apply "stash^{/2026-01-26-stage-3-ImageColorConverter}"
git stash drop "stash^{/2026-01-26-stage-3-ImageColorConverter}"
...
git stash apply "stash^{/2026-01-26-stage-4-LookupTableColorConverter}"
git stash drop "stash^{/2026-01-26-stage-4-LookupTableColorConverter}"
...
git stash apply "stash^{/2026-01-26-stage-5-PDFPageColorConverter}"
git stash drop "stash^{/2026-01-26-stage-5-PDFPageColorConverter}"
...
git stash apply "stash^{/2026-01-26-stage-6-PDFDocumentColorConverter}"
git stash drop "stash^{/2026-01-26-stage-6-PDFDocumentColorConverter}"
...
git stash apply "stash^{/2026-01-26-stage-7-ColorConverter}"
git stash drop "stash^{/2026-01-26-stage-7-ColorConverter}"
